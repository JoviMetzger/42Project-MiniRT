...
/*   Updated: 2024/04/02 17:45:30 by smclacke      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
bool	intersect_cylinder(t_ray *ray, t_objs *cylinder, t_obj_data *obj_data)
{
	...
	cylinder->center = normalize_vector(cylinder->center);
	ray->vector = cross_product(ray->vector, cylinder->center);
	// c_to_o = sub(ray.origin, cylinder.pos);
	...
	else
	{
		...
		if (obj_data->root2 < 0)
			...
			obj_data->t = (obj_data->root1 > 0 ? obj_data->root1 : obj_data->root2);
