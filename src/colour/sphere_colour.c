...
/*   Updated: 2024/04/22 22:30:52 by smclacke      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_colour	sp_colour(t_data *data, t_obj_data *obj, t_ray ray, t_objs *sp)
{
	t_colour		result;
	t_colour_vars	*vars;
	...
	vars->normal = normalize_vector(minus(vars->inter_point, sp->center));
	...
	vars->final_red = fmin(fmax(vars->final_red, sp->colour.r), 255);
	vars->final_green = fmin(fmax(vars->final_green, sp->colour.g), 255);
	vars->final_blue = fmin(fmax(vars->final_blue, sp->colour.b), 255);
